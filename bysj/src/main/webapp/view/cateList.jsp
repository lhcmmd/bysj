<%@ page contentType="text/html;charset=UTF-8"  pageEncoding="UTF-8" %>
		...
		$(function(){
			 ...
			 //datagrid添加控件
  			 $("#showAllCateTable").datagrid({
  			 	url:"${pageContext.request.contextPath}/category/queryAll",
  			 	columns:[[
  			 		{title:"复选框",field:"fx",width:1,checkbox:true},
  			 		
  			 		{title:"ID",field:"cateid",width:1},
  			 		{title:"类别名",field:"catename",width:1},
  			 		
  			 		//自定义列显示操作的删除和修改
  			 		{title:"Operate",field:"cz",
  			 			//value：字段值。row：行记录数据  index: 行索引。
  			 			formatter:function(value,row,index){
  			 				//return "<a id='delete' href='Javascript:void(0)' onclick='deleteUserByid("+row.pid+",this)'>删除/</a>"+"<a  onclick='updateUserByid("+row.pid+","+row.cate.cateid+")' href='Javascript:void(0)'>修改</a>";
  			 			}
  			 		}
  			 	]],
  			 	rownumbers:true,
			...
			$("#deleteBatchbtn").linkbutton({
				...
				onClick:function(){
					//1.获取到选中的数据的id
					var allRows = $("#showAllCateTable").datagrid("getSelections");
					if(allRows.length==0){
						...
						$.ajax({
							...
							success:function(data){
								...
								$("#showAllCateTable").datagrid("reload");
								$.messager.show({
									title:"提示",
									msg:"删除成功"
								});
	 	...
	 	$("#addDialog").dialog({
	 		...
	 		cache:false,
	  		...
	  		width:400,
	  		height:200,
	  		href:"${pageContext.request.contextPath }/view/addOneCate.jsp"
			...
			$("#searchPersons").searchbox({
				...
				searcher:function(val,name){
					if(name == "xm"){
						$("#showAllCateTable").datagrid("reload",{"name":$("#searchPersons").val()});
					}
					else if(name == "pid"){
					console.log(val);
						$("#showAllCateTable").datagrid("reload","${pageContext.request.contextPath }/person/getPersonByID?userid="+val);
					}else if(name=="sj"){
						$("#showAllCateTable").datagrid("reload","${pageContext.request.contextPath }/person/getPersonByMobile?mobile="+val);
					}
				}
			});
		...
		//修改联系人对话框
	 	$("#updateDialog").dialog({
			...
			function(res){
				...
				var pid = res[i].id;
					$("#showAllCateTable").append("<tr><td>"+res[i].id+"</td><td>"+res[i].name+"</td><td>"+res[i].mobile+"</td><td>"+res[i].birthday+"</td><td>"+"<a id='delete' href='Javascript:void(0)' onclick='deleteUserByid("+pid+",this)'>deleteOne/</a>"+"<a  onclick='updateUserByid("+pid+",this)' href='Javascript:void(0)'>updateOne</a>"+"</td></tr>");
				}
			},"json"); */
  	...
  	function deleteUserByid(userid,obj){
  		// alert(userid);
  		 $.messager.confirm("提示","确认删除吗？",function(is){
  		 	if(is){
  		 		$.get("${pageContext.request.contextPath }/person/deleteOne","pid="+userid,
  		 			function(){
  		 				$("#showAllCateTable").datagrid("reload");
 	...
 	if (value == 0)
		...
		<div id="searchMenu">
			...
			<!-- 查所有表格展示框 -->
					...
					<table id="showAllCateTable"></table>
				</div>
			</div>
			<div id="footer">
				<div id="footer_bg">
