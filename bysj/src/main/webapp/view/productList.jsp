<%@ page contentType="text/html;charset=UTF-8"  pageEncoding="UTF-8" %>
			 ...
			 //datagrid添加控件
  			 $("#showAllTable").datagrid({
  			 	...
  			 	columns:[[
  			 		...
  			 		{title:"图片",field:"picture",width:1,
  			 			formatter:function(value,row,index){
  			 				//return "<img width='30' src='${pageContext.request.contextPath}"+row.picture+"'>";
  			 			}
  			 		},
  			 		{title:"ID",field:"pid",width:1},
  			 		{title:"商品名",field:"name",width:1},
  			 		{title:"价格",field:"price",width:1},
  			 		{title:"上架时间",field:"pub_time",width:1},
  			 		{title:"库存",field:"left_num",width:1},
  			 		{title:"销量",field:"sale_num",width:1},

  			 		//自定义列显示操作的删除和修改
  			 		{title:"Operate",field:"cz",
  			 			//value：字段值。row：行记录数据  index: 行索引。
  			 			formatter:function(value,row,index){
  			 				//return "<a id='delete' href='Javascript:void(0)' onclick='deleteUserByid("+row.id+",this)'>删除/</a>"+"<a  onclick='updateUserByid("+row.id+","+row.cate.cateid+")' href='Javascript:void(0)'>修改</a>";
  			 			}
  			 		},
                    //自定义列显示操作的删除和修改
                    {title:"商品图片管理",field:"img",
                        //value：字段值。row：行记录数据  index: 行索引。
                        formatter:function(value,row,index){
                            return "<a  onclick='imgManager("+row.id+")' href='Javascript:void(0)'>图片管理</a>";
                        }
                    }
  			 	]],
  			 	rownumbers:true,
  ...
  function imgManager(pid){
      if($("#centerDiv").tabs("exists","图片管理"+pid)){
          $("#centerDiv").tabs("select","图片管理"+pid);
      }else{
          $("#centerDiv").tabs("add",{
              title:"图片管理"+pid,
              closable:true,
              href:"${pageContext.request.contextPath}/view/imgList.jsp?pid="+pid
          });
      }
